function [] = matchTrodes(centroidCoords,contactNames)


%% check whether the number of localized electrodes matches the number of electrodes. 
if ~isequal(size(centroidCoords1),length(trodeNames))
    display(sprintf('oops! \nThe number of contacts doesn"t match the number of contact names.'))
...
[idx,C] = kmeans(CentroidCoordinates(:,[1 3]),numElecs);
% make color map
...
for cnt = 1:size(CentroidCoordinates,1)
    scatter(CentroidCoordinates(cnt,1),CentroidCoordinates(cnt,3),100,cMap(idx(cnt),:),'.')
end

% looping over clusters
for cls = 1:numElecs
    scatter3(C(cls,1),C(cls,2),C(cls,3),50,cMap(cls,:),'*')
end
hold off
